...
**Platform:** iOS, Android และ **Web (PWA)** — โค้ดชุดเดียวด้วย Expo (React Native + react-native-web)
**ภาษา:** TypeScript
**สถานะ:** ฟีเจอร์หลักครบ — ต้องกรอก Supabase credentials ใน `.env` ก่อนใช้งานจริงกับ backend
...
│   = Cross-platform client        │  HTTPS  │   = Backend (managed)   │
│                                  │  ────►  │                         │
│   - Expo (RN + Web PWA export)   │         │   - PostgreSQL DB       │
...
**โปรเจกต์นี้คือ client (frontend) เท่านั้น** — ไม่มี backend code ของตัวเองใน repo
...
| **react-native-web** | ^0.21 | แปลง RN components เป็น DOM สำหรับเว็บ / PWA |
| **expo-router** | ~6 | File-based routing (มือถือ + เว็บ) |
...
| **Zustand** | ^5 | Client state (auth, ภาษา, เดือนที่เลือก) — ข้อมูลธุรกรรมอยู่ที่ React Query |
...
> **คำอธิบายสัญลักษณ์**
> ทุกไฟล์ในโปรเจกต์นี้คือ **client (frontend)** สำหรับมือถือและเว็บ การเรียก backend (Supabase) ทำผ่าน HTTPS API
...
│   └── monthFilterStore.ts       [Logic]    เดือน YYYY-MM ที่แชร์ระหว่าง Dashboard กับ History
│
│  ── Utils ──────────────────────────────────────────────────────────────────
│
├── utils/                        [Logic]
│   ├── alert.ts                  [Logic]    confirm / notify — บน web ใช้ `window.confirm` (PWA)
...
│  ── Config / Docs ──────────────────────────────────────────────────────────
│
├── public/                       [Config]   PWA: manifest, icons (คัดลอกเข้า `dist` ตอน export)
├── scripts/                      [Build]    `inject-pwa-meta.mjs`, `verify-supabase.mjs`
├── vercel.json                   [Deploy]   SPA rewrites + cache headers สำหรับโฮสต์ static
├── .env                          [Config]   ⚠️ Supabase URL + ANON KEY (gitignored)
├── .env.example                  [Config]   Template ของ .env (commit ได้)
├── app.json                      [Config]   Expo config (ชื่อ, scheme, version, web PWA meta)
...
├── package.json                  [Config]   Dependencies + scripts (`build:web`, `preview:web`)
...
| เดือนที่เลือก (แชร์ Dashboard ↔ History) | `stores/monthFilterStore.ts` |
...
### `stores/monthFilterStore.ts` — เดือนที่เลือก (global เล็กน้อย)

**บทบาท:** เก็บ `YYYY-MM` เดียวที่ **Dashboard** (`index.tsx`) และ **History** (`history.tsx`) ใช้ร่วมกัน เพื่อให้เปลี่ยนเดือนในหนึ่งหน้าแล้วอีกหน้าสอดคล้อง

**State:**
| Field | ความหมาย |
|---|---|
| `selectedMonth` | เดือนปัจจุบัน (default = เดือนนี้) |
| `setSelectedMonth` | ตั้งเดือนใหม่ |

**หมายเหตุ:** รายการธุรกรรมจริงอยู่ที่ **TanStack Query** (`hooks/useTransactions.ts`) ไม่ได้อยู่ใน Zustand — หน้า History ใช้ `useState` แยกสำหรับ filter หมวด / ประเภท (ไม่ผ่าน store นี้)
...
### Web และ PWA (production static site)

แอปรันบนเว็บได้ด้วย `npx expo start --web` (dev) — สำหรับ **ติดตั้งเป็น PWA / โฮสต์ static** ใช้ flow นี้:

| สิ่ง | ความหมาย |
|---|---|
| `app.json` → `expo.web` | ชื่อแสดงผล, theme, `output: "single"` สำหรับ export แบบหน้าเดียว |
| `npm run build:web` | รัน `expo export -p web` แล้วตามด้วย `scripts/inject-pwa-meta.mjs` — inject `<link rel="manifest">`, Apple touch icon, viewport/CSS แก้ full-bleed บน iOS PWA |
| `public/` | `manifest.webmanifest`, `icon-192.png`, `icon-512.png`, `apple-touch-icon.png` — ถ้า host ไม่คัดลอกครบ สคริปต์ inject จะพยายาม sync จาก `public/` ไป `dist/` อีกครั้ง |
| `vercel.json` | `outputDirectory: dist`, rewrite ทุก path → `index.html` (SPA), headers สำหรับ manifest และ asset แคช |
| `npm run preview:web` | ทดสอบ build แบบ static ในเครื่อง (`serve dist`) |

**Auth บนเว็บ:** `services/supabase.ts` ใช้ `detectSessionInUrl` และ `flowType: 'implicit'` เมื่อ `Platform.OS === 'web'` เพื่อรับ token หลัง OAuth redirect — บน native ใช้ PKCE + deep link ตามเดิม

**Dialog บนเว็บ:** `utils/alert.ts` ใช้ `window.confirm` / `window.alert` บน web เพราะ `Alert.alert` ของ react-native-web ไม่แสดง UI จริง
